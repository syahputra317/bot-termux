...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: OWNER GANS\n' // NAMA KONTAK OWNER
            + 'ORG:Creator ANKER;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=6285802546577:+62 858-0254-6577\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '#'
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const mod = [ownerNumber,"6285802546577@s.whatsapp.net"]
			const adminbotnumber = ["6285802546577@s.whatsapp.net"]
			const frendsowner = ["6285802546577@s.whatsapp.net"]
			const premium = ["6285802546577@s.whatsapp.net"]
			...
			switch(command) {
                ...
                break
					...
					client.sendMessage(from, qute, image, { quoted: mek, caption: 'Nihh Link Bro Download Sendiri Gan\n\nhttps://www.mediafire.com/file/pk8hozohzdc076c/VID-20210107-WA1466.mp4/file' })
					break
					case 'indo3':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6285802546577@s.whatsapp.net', options, text, {quoted: mek})
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  DARK BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner ANKER!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin ANKER!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
